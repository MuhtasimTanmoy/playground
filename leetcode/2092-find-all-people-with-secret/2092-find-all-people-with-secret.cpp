// class Solution {
// public:
//     vector<int> findAllPeople(int n, vector<vector<int>>& meets, int f) {
//         unordered_set<int> res {0, f};
//         sort(meets.begin(), meets.end(), [&](auto l, auto r) {
//             return l.back() == r.back() 
//                 ? ( res.count(l.front()) || res.count(l[1]) ) 
//                 : l.back() < r.back();
//         });
//         for (auto meet: meets) {
//             auto one = meet.front(), two = meet[1], time = meet.back();
//             if (res.count(one)) res.insert(two);
//             if (res.count(two)) res.insert(one);
//         }
//         auto s = vector<int>(res.begin(), res.end());
//         sort(s.begin(), s.end());
//         return s;
//     }
// };
